---
name: scaffold-bee-stack
description: 当用户要起一个新的"FastAPI 后端 + pnpm/Vite 前端 + Docker + GitHub Actions"的 monorepo 项目时使用。这套架构脱胎于 BeeCount-Cloud 的生产实践,适合个人/小团队 SaaS、内部工具、副业产品。触发关键词:"新建 bee-stack 项目"、"scaffold 一个 BeeCount-Cloud 风格的 monorepo"、"起一个 FastAPI + Vite 单仓"、"按 BeeCount-Cloud 模板新建项目"。不适用于纯前端项目、纯 CLI 工具、纯库项目。
---
...
当用户说"按 bee-stack 起一个新项目叫 Foo"时,按以下顺序执行:
...
```bash
cd <target>
git init && git add -A && git commit -m "feat: bee-stack 脚手架初始化"
make setup-backend
make migrate
